...
def _seconds_until(retry_at: datetime) -> float:
    """Seconds until a provider-named reset time, floored at zero."""
    if retry_at.tzinfo is None:
        retry_at = retry_at.replace(tzinfo=UTC)
    return max(0.0, retry_at.timestamp() - time.time())


def _retry_after_seconds(retry_after: str) -> float:
    """Seconds a ``Retry-After`` value asks for; 60 when it is absent or unusable."""
    try:
        seconds = float(retry_after)
    except ValueError:
        try:
            seconds = _seconds_until(parsedate_to_datetime(retry_after))
...
class CodexLLM(LLMInterface):
    ...
    def classify_failure(self, exc: BaseException) -> LLMFailureClassification | None:
        ...
        for _ in range(8):
            ...
            if isinstance(current, ProviderRateLimitResetError):
                # A usage-limit response that names its reset is raised as the defer
                # signal before any retry (``_raise_codex_quota_defer``). To the chain
                # it is still this member's explicit quota failure: cool the member
                # until that reset so another member can serve, instead of deferring
                # the whole operation on the first member's word.
                return LLMCooldownFailure(retry_after_seconds=_seconds_until(current.retry_at))
            if isinstance(current, UpstreamHTTPError) and current.status_code == 429:
                retry_after = current.headers.get("Retry-After", "")
                return LLMCooldownFailure(retry_after_seconds=_retry_after_seconds(retry_after))
